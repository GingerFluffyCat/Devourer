...
WfbngLink::WfbngLink(JNIEnv *env, jobject context)
    ...
    initAgg();
    // NOTE: this used to construct a default (null) Logger_t and hand THAT to
    // WiFiDriver while leaving the `log` class member null too -- harmless only
    // because nothing on this path called into it. The new devourer/src Logger is
    // dereferenced unconditionally (no null-checks), so both need a real instance.
...
int WfbngLink::run(JNIEnv *env, jobject context, jint wifiChannel, jint bw, jint fd) {
    ...
    // CreateRtlDevice returns the chip-agnostic IRtlDevice; WFB-ng targets Jaguar1
    // (8812AU) only, so downcast to the concrete type rtl_devices stores. Always a
    // RtlJaguarDevice in practice for this chip family.
    {
        std::unique_ptr<IRtlDevice> dev = wifi_driver->CreateRtlDevice(dev_handle);
        RtlJaguarDevice *jaguar1 = dev ? dynamic_cast<RtlJaguarDevice *>(dev.get()) : nullptr;
        if (!jaguar1) {
            libusb_exit(ctx);
        ...
        dev.release();
        rtl_devices[fd] = std::shared_ptr<RtlJaguarDevice>(jaguar1);
    }
...
void WfbngLink::start_link_quality_thread(int fd) {
    ...
    rtl_devices.at(fd)->SetTxPowerIndexOverride(adaptive_tx_power);
}
...
extern "C" JNIEXPORT void JNICALL Java_com_openipc_wfbngrtl8812_WfbNgLink_nativeSetTxPower(JNIEnv *env,
    ...
    if (link->current_fd != -1 && link->rtl_devices.find(link->current_fd) != link->rtl_devices.end()) {
        link->rtl_devices.at(link->current_fd)->SetTxPowerIndexOverride(power);
    }
